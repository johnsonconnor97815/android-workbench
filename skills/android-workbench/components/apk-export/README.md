...
# Android APK Export Component

`COMPONENT_DIR` is the directory containing this README.

Use the bundled `scripts/apk_pull.py`, resolving its path relative to this
component directory, regardless of the current working directory. It needs Python 3.10+,
...
2. Check tools and locate the package:
   ...
   ```bash
   python3 "$COMPONENT_DIR/scripts/apk_pull.py" doctor
   python3 "$COMPONENT_DIR/scripts/apk_pull.py" devices --package com.example.app
   ```
...
4. Choose a new export directory inside the requested destination. Preserve
   ...
   ```bash
  python3 "$COMPONENT_DIR/scripts/apk_pull.py" pull \
...
5. Check the exit code and JSON result. `pull` verifies the staged archive before
   ...
   ```bash
    python3 "$COMPONENT_DIR/scripts/apk_pull.py" verify /destination/com.example.app
   ```
...
installing or publishing this plugin.
