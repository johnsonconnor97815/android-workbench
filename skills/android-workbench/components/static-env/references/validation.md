...
- 入口 Skill 格式校验通过，Python 脚本语法校验通过。
...
以上是发布前开发验证的摘要。原始日志、下载缓存、APK 和本地构建产物不随插件分发。可以使用 `scripts/test_setup.py` 与 `scripts/smoke.py` 在自己的工作区重做对应检查；仓库的 CI 另外验证各 Agent 安装后的文件完整性。
...
- 7 项 MCP 本地行为测试及原有 6 项安装器测试通过；9 种 Agent 的实际入口 Skill 复制安装与资源哈希检查通过。客户端 MCP 配置已生成；这不等于已实测每种客户端的 MCP 运行时。
