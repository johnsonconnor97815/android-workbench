...
先按项目指令激活已有环境，例如 `source .envrc`。`COMPONENT_DIR` 是当前安装的静态环境组件路径。

```bash
python3 "$COMPONENT_DIR/scripts/mcp_setup.py" plan --workspace "$PWD"
python3 "$COMPONENT_DIR/scripts/mcp_setup.py" install --workspace "$PWD"
python3 "$COMPONENT_DIR/scripts/mcp_setup.py" check --workspace "$PWD"
python3 "$COMPONENT_DIR/scripts/mcp_setup.py" configure --workspace "$PWD" --client codex
```
...
```bash
python3 "$COMPONENT_DIR/scripts/mcp_setup.py" configure --workspace "$PWD" --client codex --enable-jadx
```
...
```bash
python3 "$COMPONENT_DIR/scripts/mcp_smoke.py" --workspace "$PWD" \
...
```bash
python3 "$COMPONENT_DIR/scripts/mcp_probe.py" \
